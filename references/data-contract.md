...
顶层可增加 `card` 对象；缺省时旧版 plan 仍可检查和导出旅行手帐。新卡片在选择内容后保存完整 `card.draft`，由用户审核；不必填写 `sections`。字段出现时严格校验类型与子字段，未知字段拒绝。`schema_version` 保持 1。

`draft` 存在时优先使用，不拼接结构化栏目、人数、预算、系统状态或署名。存在但为空白是错误，不回退到旧内容。文本按原文导出，不自动脱敏或润色；必要的隐私处理、事实核对、未定事项措辞须在审核前完成。署名由图片设计单独标记，不能用署名区域藏入其他内容。无 `draft` 的旧数据仍可通过 `sections` 生成原模板；两者都没有时先起草，不能自动填满卡片。
...
| `packing` | 非空物品名称数组；选择该栏目后展示，物品太多时先精简审核稿，不自动附加文字文件或图片 |
...
结构化数据仅作起草素材或旧模板输入。自由稿由 Agent 从行程和对话中选取事实，不能自动带入预约确认依据、证件、联系方式或具体住宿地址；脚本的文字匹配不等于事实或隐私审查。只有用户想写且已有可靠数据时，才加入投票、分工或 AA，不由人数自动开启或禁用栏目。详见 delivery.md。
